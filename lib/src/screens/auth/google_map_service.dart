import 'package:kkukku/src/models/address_from_location_model.dart';
import 'package:kkukku/src/models/location_from_address_model.dart';
...
// 주소정보를 입력해서 위도 경도 정보를 가져온다
  static Future<LocationFromAddress> getLocationFromAddress(
    ...
    final LocationFromAddress locationFromAddress =
        LocationFromAddress.fromJson(response.data);
...
// 위도 경도 정보를 이용해서 주소 정보를 찾는 함수
  static Future<AddressFromLocation> getAddressFromLocation(
    ...
    final AddressFromLocation addressFromLocation =
        AddressFromLocation.fromJson(response.data);
...
// 위도 경도 정보를 이용해서 주소 정보를 찾는 함수
  static Future<List<AddressFromLocation>> getAddressFromLocation5(
      double lat, double lng) async {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      String url =
          '$baseUrl?latlng=${lat + 0.01 * y},${lng + 0.01 * x}&key=$googleApiKey&language=ko';
      var response = await Dio().get(url).catchError((e) {
        logger.e(e.message);
      });
